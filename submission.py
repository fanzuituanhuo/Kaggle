...
核心思路：
1. 专家 A：CRNN (时间序列专家) - 捕捉旋律时序特征，输出 256 维特征
2. 专家 B：ResNet-18 (空间纹理专家) - 捕捉声纹纹理特征，输出 512 维特征
3. 融合策略：特征拼接 (768维) + 浅层 MLP 分类器

优势：
- 消除单模型的结构性盲区
- 低训练开销（冻结专家，只训练 ~10 万参数的分类头）
- 稳定的泛化性能
...
# timm for pretrained models (ResNet-18)
import timm
...
def remove_silence(audio, sr, top_db=30, frame_length=2048, hop_length=512, min_duration=0.1, verbose=False):
    ...
    if trimmed_duration < min_duration:
        if verbose:
            print(f"    警告: 切除后时长 {trimmed_duration:.2f}s 太短，保留原音频")
        return audio
    ...
    if verbose:
        ...
        print(f"原始时长: {original_duration:.2f}s, "
              f"切除首部: {removed_start:.2f}s, "
              f"切除尾部: {removed_end:.2f}s, "
              f"保留: {trimmed_duration:.2f}s")
...
def augment_single_audio(audio, sr, augmentation_type, param):
    ...
    try:
        ...
        print(f"    增强失败 ({augmentation_type}, param={param}): {e}")
        return None
...
def _augment_worker(row_dict, strategies, output_dir, verbose=False):
    ...
    try:
        ...
        for aug_type, aug_param in strategies:
            ...
            if verbose:
                print(f"  生成: {aug_type}({aug_param}) -> {new_filename}")
    except Exception as e:
...
# ==========================================
# 5. 按参与者分组的 Train/Val/Test 划分
# ==========================================
def split_by_participant(df, train_ratio=0.7, val_ratio=0.15, seed=42):
    """按参与者 ID 进行分组划分"""
...
class SpectrogramDataset(Dataset):
    """PyTorch Dataset: 用于加载频谱图数据"""
    def __init__(self, spectrograms, labels, apply_spec_augment=False,
    ...
    def __getitem__(self, idx):
        spectrogram = self.spectrograms[idx].unsqueeze(0)
...
# ----------------------
# 7.2 CRNN 主干与特征提取器 (专家 A - 时间序列专家)
# ----------------------
class CRNNBackbone(nn.Module):
    """
    通用的 CNN+GRU 主干，可输出时序聚合后的特征。
    """
    def __init__(self, input_height=128, input_width=512,
                 rnn_hidden_size=128, rnn_num_layers=2, dropout_rate=0.5,
                 use_attention=True, attention_hidden_size=128):
        super().__init__()
        ...
        # CNN Dropout 层（防止过拟合）
        self.dropout_cnn1 = nn.Dropout2d(0.5)  # 在 pool2 后
        self.dropout_cnn2 = nn.Dropout2d(0.5)  # 在 pool3 后
        ...
        self.gru_output_size = rnn_hidden_size * 2  # 256 (双向)
        ...
        # 输出特征维度
        self.output_dim = self.gru_output_size  # 256
    ...
    def forward_features(self, x):
        ...
        x = self.pool2(x)
        x = self.dropout_cnn1(x)  # CNN Dropout

        x = F.relu(self.bn3_1(self.conv3_1(x)))
        x = F.relu(self.bn3_2(self.conv3_2(x)))
        x = self.pool3(x)
        x = self.dropout_cnn2(x)  # CNN Dropout
...
# ----------------------
# 7.3 ResNet-18 特征提取器 (专家 B - 空间纹理专家)
# ----------------------
class ResNet18FeatureExtractor(nn.Module):
    """
    ResNet-18 特征提取器 (空间纹理专家)

    输入: (batch, 1, 128, 512)
    输出: (batch, 512) - 512 维特征向量
    """
    def __init__(self, pretrained=True):
        ...
        self.output_dim = self.backbone.num_features  # 512

    def forward(self, x):
        return self.backbone(x)  # (batch, 512)
...
class FusionClassifier(nn.Module):
    ...
    输入: (batch, 768) - CRNN(256) + ResNet(512)
...
class FeatureFusionEnsemble(nn.Module):
    """
    互补特征融合集成模型 (Complementary Feature Fusion Ensemble)

    结构:
    - 专家 A: CRNN (时间序列专家) -> 256 维
    - 专家 B: ResNet-18 (空间纹理专家) -> 512 维
    - 融合: 特征拼接 (768 维) + 浅层 MLP 分类器

    训练策略:
    - 阶段 1: 分别训练两个专家模型
    - 阶段 2: 冻结专家，只训练融合分类器
    """
    def __init__(self, num_classes=8, freeze_experts=True, dropout_rate_fusion=0.5, use_attention=True):
        super().__init__()
        ...
        self.crnn_expert = CRNNFeatureExtractor(
            ...
            rnn_hidden_size=128,
            rnn_num_layers=2,
            dropout_rate=dropout_rate_fusion,  # 使用融合模型的 dropout 率
            use_attention=use_attention,
            attention_hidden_size=128
        )

        # 专家 B: ResNet-18 (空间纹理专家)
        self.resnet_expert = ResNet18FeatureExtractor(pretrained=True)

        # 融合分类器
        fusion_input_dim = self.crnn_expert.output_dim + self.resnet_expert.output_dim  # 256 + 512 = 768
    ...
    def freeze_expert_parameters(self):
        ...
        for param in self.resnet_expert.parameters():
            param.requires_grad = False
        print("✓ 已冻结 CRNN 和 ResNet-18 专家模型参数")
    ...
    def load_expert_weights(self, crnn_path=None, resnet_path=None, device='cpu'):
        """
        加载预训练的专家模型权重

        参数:
            crnn_path: CRNN 模型权重路径
            resnet_path: ResNet-18 模型权重路径
            device: 设备
        """
        if crnn_path and os.path.exists(crnn_path):
            ...
            # 从完整的 CRNNClassifier 中提取特征提取部分
            state_dict = checkpoint['model_state_dict']

            # 过滤掉分类头的权重
            feature_extractor_dict = {}
            for key, value in state_dict.items():
                # 排除原始分类头 (fc1, fc2, bn_fc1, bn_fc2)
            ...
            print(f"  ✓ CRNN 专家加载成功 (加载了 {len(feature_extractor_dict)} 个参数)")

        if resnet_path and os.path.exists(resnet_path):
            print(f"加载 ResNet-18 权重: {resnet_path}")
            ...
            for key, value in state_dict.items():
                if key.startswith('backbone.'):
                    new_key = key  # 保持 backbone. 前缀
                    feature_extractor_dict[new_key] = value

            self.resnet_expert.load_state_dict(feature_extractor_dict, strict=False)
            print(f"  ✓ ResNet-18 专家加载成功 (加载了 {len(feature_extractor_dict)} 个参数)")

    def forward(self, x):
        # 提取 CRNN 特征 (时间序列)
        crnn_features = self.crnn_expert(x)  # (batch, 256)

        # 提取 ResNet 特征 (空间纹理)
        resnet_features = self.resnet_expert(x)  # (batch, 512)

        # 特征拼接
        fused_features = torch.cat([crnn_features, resnet_features], dim=1)  # (batch, 768)
...
# ----------------------
# 7.6 完整的 CRNN 分类器 (用于单独训练专家 A)
# ----------------------
class CRNNClassifier(CRNNBackbone):
    """完整的 CRNN 分类器（用于单独训练专家 A）"""
    def __init__(self, num_classes=8, input_height=128, input_width=512,
                 rnn_hidden_size=128, rnn_num_layers=2, dropout_rate=0.7,
                 use_attention=True, attention_hidden_size=128):
        super().__init__(input_height=input_height,
                         input_width=input_width,
                         rnn_hidden_size=rnn_hidden_size,
...
# ----------------------
# 7.7 ResNet-18 分类器 (用于单独训练专家 B)
# ----------------------
class AudioResNet18(nn.Module):
    """ResNet-18 分类器（用于单独训练专家 B）"""
...
# 10. 训练函数
# ==========================================
def train_single_expert(model, train_loader, val_loader, num_epochs=50, learning_rate=0.001,
                        device='cuda', early_stopping_patience=10, model_name='model',
                        class_weights=None):
    """训练单个专家模型"""
    print(f"\n{'='*60}")
    ...
    for epoch in range(num_epochs):
        ...
        if patience_counter >= early_stopping_patience:
            print(f"\n早停触发！验证准确率 {early_stopping_patience} 轮未提升")
            break

    if best_model_state is not None:
        model.load_state_dict(best_model_state)
        print(f"\n恢复最佳模型 (验证准确率: {best_val_acc:.2f}%)")

    return model, history


def train_fusion_classifier(ensemble_model, train_loader, val_loader, num_epochs=15, learning_rate=0.001,
                            device='cuda', early_stopping_patience=5, class_weights=None):
    """
    训练融合分类器（冻结专家模型）
    """
    print(f"\n{'='*60}")
    print("阶段 2: 训练融合分类器 (冻结专家)")
    print(f"{'='*60}")
    ...
    # 冻结专家参数
    ensemble_model.freeze_expert_parameters()

    # 统计可训练参数
    total_params, trainable_params = count_parameters(ensemble_model)
    print(f"模型总参数: {total_params:,}")
    print(f"可训练参数: {trainable_params:,} (仅融合分类器)")

    # 只优化融合分类器的参数
    optimizer = optim.Adam(ensemble_model.fusion_classifier.parameters(), lr=learning_rate, weight_decay=1e-4)
    weight_tensor = class_weights.to(device) if class_weights is not None else None
    criterion = nn.CrossEntropyLoss(weight=weight_tensor)
    ...
    for epoch in range(num_epochs):
        ...
        for spectrograms, labels in train_loader:
            ...
            optimizer.zero_grad()
            outputs = ensemble_model(spectrograms)
            loss = criterion(outputs, labels)
            loss.backward()
            optimizer.step()
...
# 11. 主流程：互补特征融合集成
# ==========================================
def run_feature_fusion_ensemble_pipeline(data_dir=None, augment=True, target_total=4000,
                                          ...
                                          resnet_model_path='resnet18_model_mel_torchaudio.pth',
    ...
    参数:
        ...
        resnet_model_path: ResNet-18 模型权重路径
    ...
    print("="*80)
    print("\n架构说明:")
    print("  - 专家 A: CRNN (时间序列专家) -> 256 维特征")
    print("  - 专家 B: ResNet-18 (空间纹理专家) -> 512 维特征")
    print("  - 融合: 特征拼接 (768 维) + 浅层 MLP 分类器 (~10 万参数)")
    print("="*80)
    ...
    if augment:
        spectrogram_save_path = 'mel_spectrograms_augmented_torchaudio.npz'
    else:
        spectrogram_save_path = 'mel_spectrograms_original_torchaudio.npz'

    if not force_regenerate and os.path.exists(spectrogram_save_path):
        print(f"✓ 检测到已有频谱图文件: {spectrogram_save_path}")
        print("直接加载...")

        data = np.load(spectrogram_save_path, allow_pickle=True)
        ...
        for file_id in file_ids:
            parts = str(file_id).replace('.wav', '').split('_')
            participants.append(parts[0] if len(parts) >= 1 else 'Unknown')
        ...
        print(f"加载完成: {len(spectrograms)} 个频谱图")
    else:
        # 加载原始数据
        ...
        spectrograms, labels, valid_df = extract_spectrograms_from_dataframe(
            df=df_data,
    ...
    # ==========================================
    print("\n" + "="*60)
    print("步骤 2: 按参与者划分数据集 (Logo Principle)")
    print("="*60)

    splits = split_by_participant(valid_df, train_ratio=0.7, val_ratio=0.15, seed=42)

    train_idx = splits['train']
    val_idx = splits['val']
    test_idx = splits['test']

    print(f"Train: {len(train_idx)} 样本, {len(splits['train_parts'])} 个参与者")
    print(f"Val:   {len(val_idx)} 样本, {len(splits['val_parts'])} 个参与者")
    print(f"Test:  {len(test_idx)} 样本, {len(splits['test_parts'])} 个参与者")
    ...
    try:
        ...
        happy_idx = SONG_TO_LABEL.get('Happy', None)
        if happy_idx is not None:
            inv_freq[happy_idx] *= 1.0  # 适度提升 Happy 权重

        class_weights = torch.tensor(inv_freq, dtype=torch.float32)
        print(f"类别权重（含 Happy 加权 1.2x）: {inv_freq}")
    except Exception as e:
        print(f"⚠ 类别权重计算失败，使用均等权重: {e}")
        class_weights = None
    ...
    if not skip_expert_training:
        ...
        crnn_model, crnn_history = train_single_expert(
            model=crnn_model,
        ...
        # 3.2 训练 ResNet-18 专家
        print("\n--- 训练专家 B: ResNet-18 (空间纹理专家) ---")
        resnet_model = AudioResNet18(
            num_classes=num_classes,
            pretrained=True
        )

        resnet_model, resnet_history = train_single_expert(
            model=resnet_model,
            train_loader=train_loader,
            val_loader=val_loader,
            num_epochs=num_epochs_experts,
        ...
        print(f"✓ ResNet-18 模型已保存到: {resnet_model_path}")

        # 评估 ResNet
        print("\n--- ResNet-18 专家测试评估 ---")
        resnet_test_acc, _, _ = evaluate_model(resnet_model, test_loader, LABEL_TO_SONG, device=device)

        print("\n" + "="*60)
        print("专家模型训练完成")
        print(f"  CRNN 测试准确率: {crnn_test_acc:.2f}%")
        print(f"  ResNet-18 测试准确率: {resnet_test_acc:.2f}%")
        print("="*60)
    ...
    # 训练融合分类器
    ensemble_model, fusion_history = train_fusion_classifier(
    ...
    print(f"  - ResNet-18 专家: {resnet_model_path}")
...
if __name__ == '__main__':
    run_feature_fusion_ensemble_pipeline(
        augment=True,                    # 使用数据增强
        target_total=4000,               # 增强后目标样本数
        batch_size=32,                   # 批次大小
        num_epochs_experts=60,           # 专家模型训练轮数
        num_epochs_fusion=15,            # 融合分类器训练轮数
        learning_rate=0.0005,            # 学习率
        n_jobs=8,                        # 并行进程数
        force_regenerate=False,          # 是否强制重新生成频谱图
        skip_expert_training=False,      # 是否跳过专家训练（使用已有权重）
        crnn_model_path='crnn_model_mel_torchaudio.pth',
        resnet_model_path='resnet18_model_mel_torchaudio.pth',
        dropout_rate_fusion=0.6,         # 融合分类器的 dropout 率
        dropout_rate_expert=0.7,         # 专家模型的 dropout 率（GRU + 分类头）
        use_attention=False               # 是否使用时序注意力机制
    )
